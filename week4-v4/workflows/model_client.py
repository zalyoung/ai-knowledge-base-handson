...
def chat_json(
    ...
    """对话请求并解析 JSON 响应，返回 (解析结果, 用量) 元组。

    自动处理 LLM 响应中可能包含的 markdown 代码块标记。
    ...
    Returns:
        (解析后的 JSON 对象, Usage 用量统计) 元组。

    Raises:
        json.JSONDecodeError: 响应内容无法解析为 JSON。

    Example:
        >>> data, usage = chat_json('返回 {"key": "value"}')
        >>> print(data["key"])
    """
    text, usage = chat(
        prompt=prompt,
...
def accumulate_usage(tracker: Dict[str, Any], usage: Usage) -> None:
    """将单次调用的用量累加到 tracker 字典。

    Args:
        tracker: 用量追踪字典，包含 total_tokens, input_tokens,
            output_tokens, call_count 等字段。
        usage: 单次调用的 Usage 统计。
    """
    tracker["total_tokens"] = tracker.get("total_tokens", 0) + usage.total_tokens
    tracker["input_tokens"] = tracker.get("input_tokens", 0) + usage.prompt_tokens
    tracker["output_tokens"] = tracker.get("output_tokens", 0) + usage.completion_tokens
